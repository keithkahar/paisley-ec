  ...
  return (
    <PhoneFrame bg="bg-[color:var(--wordie-soft)]">
      <AppHeader
        ...
        right={
          <span className="text-xs font-bold text-[color:var(--wordie)]">
            {idx + 1}/{total}
          </span>
        }
      />
      ...
      <section className="px-5 mt-5">
        ...
        >
          ...
          >
            ...
            >
              ...
              <div className="flex-1 grid place-items-center text-center">
                <div>
                  ...
                  <h2 className="text-4xl font-bold text-[color:var(--wordie)]">{card.word}</h2>
                  <p className="text-sm text-muted-foreground mt-1.5 font-mono">{card.ipa}</p>
                  <button
                    type="button"
                    onClick={(e) => e.stopPropagation()}
                    className="mt-4 inline-flex items-center gap-2 rounded-full px-4 py-2 text-white text-sm font-bold shadow"
            ...
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold rounded-full px-2 py-1 bg-white/20">
                  {card.word}
                </span>
              ...
              <div className="flex-1 flex flex-col justify-center gap-4 mt-2">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider opacity-70">Meaning</p>
                  <p className="text-lg font-bold leading-snug mt-1">{card.meaning}</p>
                </div>
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-wider opacity-70">Translation</p>
                  <p className="text-base font-bold mt-1">{card.translation}</p>
                </div>
      ...
      <section className="px-5 mt-5 grid grid-cols-2 gap-3">
        <button
          type="button"
          onClick={() => next(false)}
          className="rounded-2xl bg-white border border-border py-3.5 font-bold text-sm inline-flex items-center justify-center gap-2 active:scale-[0.98]"
        >
        ...
        <button
          type="button"
          onClick={() => next(true)}
          className="rounded-2xl py-3.5 font-bold text-sm text-white inline-flex items-center justify-center gap-2 shadow-md active:scale-[0.98]"
          style={{ background: "var(--wordie)" }}
        >
      ...
      <section className="px-5 mt-4 mb-8 text-center">
        <p className="text-xs text-muted-foreground">
          <span className="font-bold text-[color:var(--wordie)]">{known}</span> known this session
        </p>
